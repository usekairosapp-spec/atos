-- Deixa o motivo do aviso como uma frase completa e autoexplicativa, pra
-- nao ficar redundante com o texto fixo do app nos dois casos possiveis
-- (ja esta nesta mesma escala / tem outro compromisso no mesmo horario).
...
begin
  ...
  select
    ...
    coalesce(
      (select 'já está escalado(a) nesta mesma escala, em outra função' from public.schedule_assignments same where same.department_schedule_id = target_schedule_id and same.user_id = dm.user_id limit 1),
      (
        select 'já está escalado(a) em outro compromisso nesse mesmo horário (' || d2.name || ', ' || to_char(other_s.starts_at at time zone 'America/Sao_Paulo', 'DD/MM') || ')'
        from public.schedule_assignments other_sa
        join public.department_schedules other_ds on other_ds.id = other_sa.department_schedule_id
