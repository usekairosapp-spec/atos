-- Permite ao lider do setor trocar quem esta em uma posicao especifica de
-- uma escala, direto pela lista "Por pessoa", sem precisar da pessoa nova
-- aceitar um convite. Ao contrario da troca normal (get_sector_swap_candidates),
-- aqui a lista traz TODO MUNDO do setor, sinalizando quem ja tem outro
-- compromisso no mesmo horario (has_conflict) — o lider decide se troca
-- mesmo assim; a funcao nao bloqueia por conflito, so informa.
...
revoke all on function public.get_department_swap_candidates_for_leader(uuid) from public;
grant execute on function public.get_department_swap_candidates_for_leader(uuid) to authenticated;
