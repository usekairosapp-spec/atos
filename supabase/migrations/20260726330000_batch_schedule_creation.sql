-- Cria varias escalas de uma vez (mesmo setor, mesmo horario/local/equipe,
-- datas escolhidas manualmente) e ja publica cada uma individualmente.
-- Cada escala segue o fluxo normal: nasce como rascunho, recebe a equipe,
-- e so entao e marcada como publicada — isso garante que os gatilhos de
-- notificacao existentes (notify_on_schedule_publish) disparem um aviso
-- por escala, exatamente como se cada uma tivesse sido criada e publicada
-- manualmente, uma de cada vez.
...
revoke all on function public.create_department_schedules_batch(uuid, date[], text, time, time, text, text, jsonb) from public;
grant execute on function public.create_department_schedules_batch(uuid, date[], text, time, time, text, text, jsonb) to authenticated;
