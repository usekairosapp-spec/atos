create or replace function public.get_my_calendar_assignments(target_church_id uuid)
returns table (
  assignment_id uuid,
...
as $$
  select sa.id, sa.status, ds.id, s.title, s.starts_at, s.ends_at, s.location, d.name, p.name
...
revoke all on function public.get_my_calendar_assignments(uuid) from public;
grant execute on function public.get_my_calendar_assignments(uuid) to authenticated;
