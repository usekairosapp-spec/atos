...
export function CentralBottomNavigation({ unreadCount, pendingMembershipCount }: { unreadCount: number; pendingMembershipCount: number }) {
  ...
  return (
    <nav className="lg:hidden fixed inset-x-0 bottom-0 z-30 mx-auto flex max-w-3xl justify-around border-t border-[#e2e7ee] bg-white px-1 pt-2 shadow-[0_-8px_25px_rgba(22,38,55,.06)] dark:border-white/10 dark:bg-[#0c232c] [padding-bottom:max(.5rem,env(safe-area-inset-bottom))]" aria-label="Navegação da Central">
      {navigation.map(({ href, label, icon: Icon, badge }) => {
        ...
        return (
          <Link
            ...
            className={`flex min-h-12 min-w-12 flex-col items-center justify-center gap-1 px-1 text-[10px] font-medium sm:min-w-14 sm:text-[11px] relative ${active ? "text-[#277ad8]" : "text-[#6b767d]"}`}
            href={href}
            key={href}
          >
            <div className="relative">
              ...
              {count ? (
                <span className={`absolute -right-2 -top-2 grid h-5 min-w-5 place-items-center rounded-full px-1 text-[8px] font-bold ${badge === "requests" ? "bg-amber-400 text-[#0c232c]" : "bg-red-500 text-white"}`}>
                  {count > 9 ? "9+" : count}
                </span>
              ) : null}
            </div>
