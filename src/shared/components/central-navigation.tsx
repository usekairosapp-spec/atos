  ...
  return <nav className="mt-8 space-y-2" aria-label="Navegação da Central">
    {navigation.map(({ href, label, icon: Icon, badge }) => {
      ...
      return <Link
        ...
        className={`flex min-h-12 items-center justify-between rounded-xl px-4 transition ${active ? "bg-white/12 text-white shadow-sm ring-1 ring-white/10" : "text-violet-200 hover:bg-white/5 hover:text-white"}`}
        href={href}
        key={href}
      >
        <span className="flex items-center gap-3"><Icon size={20} />{label}</span>
        {count ? <span className={`grid h-6 min-w-6 place-items-center rounded-full px-1 text-xs font-bold ${badge === "requests" ? "bg-amber-400 text-[#100c2c]" : "bg-red-500 text-white"}`}>{count > 9 ? "9+" : count}</span> : null}
      </Link>;
    })}
  </nav>;
}
