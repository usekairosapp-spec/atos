...
export function BottomNavigation({ role }: { role: ViewerRole }) {
  ...
  return <nav className="fixed inset-x-0 bottom-0 z-30 mx-auto flex max-w-3xl justify-around border-t border-[#e7e2ee] bg-white px-1 pt-2 shadow-[0_-8px_25px_rgba(34,22,55,.06)] [padding-bottom:max(.5rem,env(safe-area-inset-bottom))]" aria-label="Navegação principal">{navigationItems.map(([href, label, Icon]) => { const baseHref = href.split("?")[0]; const active = baseHref === "/painel" ? pathname === baseHref : pathname.startsWith(baseHref); return <Link aria-current={active ? "page" : undefined} className={`flex min-h-12 min-w-12 flex-col items-center justify-center gap-1 px-1 text-[10px] font-medium sm:min-w-14 sm:text-[11px] ${active ? "text-[#6827d8]" : "text-[#6f6b7d]"}`} href={href} key={href}><Icon fill={active ? "currentColor" : "none"} size={21} /><span>{label}</span></Link>; })}</nav>;
}
