...
export default async function LoginPage({ searchParams }: LoginPageProps) {
  ...
  return (
    <main className="grid min-h-screen bg-[var(--background)] lg:grid-cols-[1fr_1.15fr]">
      <aside className="relative hidden overflow-hidden bg-[radial-gradient(circle_at_top_left,#7440d1,#271746_45%,#0d0a14)] p-14 text-white lg:flex lg:flex-col lg:justify-between">
        <div className="absolute -right-32 top-20 h-96 w-96 rounded-full bg-purple-400/20 blur-3xl" /><AtosBrand light />
        <div className="max-w-lg">
          <p className="text-sm font-semibold uppercase tracking-[.22em] text-violet-200">Sua equipe em sintonia</p>
          <h1 className="mt-4 text-5xl font-bold leading-tight">Cada pessoa no lugar certo, na hora certa.</h1>
        </div>
        <p className="text-sm text-violet-200">Técnica · Mídia · Oficiais · Louvor · Recepção · Kids</p>
      </aside>
      ...
      <section className="relative flex items-center justify-center px-5 py-10 sm:px-10"><div className="absolute right-5 top-5"><ThemeToggle /></div>
        <div className="premium-card w-full max-w-md rounded-[2rem] p-7 sm:p-10">
          <div className="mb-10 lg:hidden"><AtosBrand /></div>
          ...
          <form action={login} className="mt-5 space-y-5">
            <label className="block font-medium">E-mail
              <span className="mt-2 flex min-h-14 items-center gap-3 rounded-xl border border-[#dcd7e5] px-4 focus-within:border-[#6827d8]">
                <Mail size={19} aria-hidden="true" className="text-[#817c8d]" />
            ...
            <label className="block font-medium">Senha
              <span className="mt-2 flex min-h-14 items-center gap-3 rounded-xl border border-[#dcd7e5] px-4 focus-within:border-[#6827d8]">
                ...
                <Eye size={19} aria-label="Mostrar senha" className="text-[#817c8d]" />
              </span>
            </label>
            <div className="text-right"><Link className="font-semibold text-[#6827d8]" href="/recuperar-senha">Esqueci minha senha</Link></div>
            <button className="min-h-14 w-full rounded-xl bg-[#6827d8] px-5 font-semibold text-white shadow-lg hover:bg-[#5720bd]" type="submit">Entrar</button>
          </form>
          </details>
