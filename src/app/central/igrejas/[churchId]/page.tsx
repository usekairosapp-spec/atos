...
export default async function CentralChurchBrandingPage({ params, searchParams }: PageProps) {
  ...
  return <main className="mx-auto max-w-5xl px-5 py-8 sm:px-8"><Link className="inline-flex items-center gap-2 font-semibold text-[var(--brand)]" href="/central/igrejas"><ArrowLeft size={18} />Voltar para igrejas</Link><div className="mt-6 flex gap-3"><Link className="inline-flex items-center gap-2 rounded-lg bg-[var(--brand)] px-4 py-2 font-semibold text-white transition hover:opacity-90" href={`/central/igrejas/${churchId}/admins`}>Gerenciar administradoras</Link></div><p className="mt-7 text-sm font-semibold uppercase tracking-[.15em] text-[var(--brand)]">Identidade visual</p><h1 className="mt-2 text-3xl font-bold">{church.name}</h1><AuthMessage {...message} />
    <section className="premium-card mt-7 overflow-hidden rounded-[2rem]"><div className="relative h-56 bg-[linear-gradient(135deg,var(--church-color),#17112d)]" style={{ "--church-color": church.primary_color } as React.CSSProperties}>{coverUrl ? <Image alt={`Capa de ${church.name}`} className="object-cover opacity-75" fill sizes="1000px" src={coverUrl} unoptimized /> : null}<div className="absolute inset-0 bg-gradient-to-t from-black/75 to-transparent" /><div className="absolute bottom-6 left-6 flex items-center gap-4">{logoUrl ? <Image alt={`Logo de ${church.name}`} className="h-20 w-20 rounded-2xl bg-white object-contain p-1" height={80} src={logoUrl} width={80} unoptimized /> : <span className="rounded-2xl bg-white p-3"><AtosBrand compact /></span>}<strong className="text-2xl text-white">{church.name}</strong></div></div>
