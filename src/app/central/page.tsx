...
export default async function PlatformCentralPage({ searchParams }: PageProps) {
  ...
  return (
    <main className="mx-auto max-w-7xl px-5 py-8 sm:px-8">
      <p className="text-sm font-semibold uppercase tracking-[.15em] text-[var(--brand)]">Central da plataforma</p><h1 className="mt-2 text-3xl font-bold">Olá, {viewer?.profile.fullName.split(" ")[0]}</h1><p className="mt-2 text-[var(--muted)]">Acompanhe quem utiliza o Kairos Escala e defina os responsáveis por cada igreja.</p>
      <AuthMessage {...message} />
      <section className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><article className="premium-card rounded-2xl p-6 shadow-sm"><Building2 className="text-[#277ad8]" /><strong className="mt-3 block text-3xl">{churches?.length ?? 0}</strong><span className="text-sm text-[var(--muted)]">Igrejas cadastradas</span></article><article className="premium-card rounded-2xl p-6 shadow-sm"><UsersRound className="text-[#277ad8]" /><strong className="mt-3 block text-3xl">{eligibleUsers.length}</strong><span className="text-sm text-[var(--muted)]">Usuários do aplicativo</span></article><article className="premium-card rounded-2xl p-6 shadow-sm"><UserCog className="text-[#277ad8]" /><strong className="mt-3 block text-3xl">{activeMemberships.filter((item) => item.role === "church_admin").length}</strong><span className="text-sm text-[var(--muted)]">Administradores de igreja</span></article><Link className="rounded-2xl bg-[#277ad8] p-6 text-white shadow-sm transition hover:bg-[#2069b8]" href="/central/solicitacoes"><UserCog /><strong className="mt-3 block text-3xl">{pendingMemberships.length}</strong><span className="text-sm text-blue-100">{pendingMemberships.length === 1 ? "Solicitação aguardando análise" : "Solicitações aguardando análise"}</span><span className="mt-3 block text-sm font-bold">Analisar agora →</span></Link></section>
