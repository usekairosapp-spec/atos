...
export default async function SchedulesPage({ searchParams }: PageProps) {
  ...
  return <main className="mx-auto max-w-4xl px-4 py-7 sm:px-8"><div className="flex items-center justify-between gap-4"><h1 className="text-3xl font-bold">{view === "minhas" ? "Minhas escalas" : "Escalas do setor"}</h1>{canManageSchedules && view === "setor" ? <Link aria-label="Nova escala" className="grid h-12 w-12 place-items-center rounded-full bg-[var(--church-brand)] text-white shadow-lg" href="/painel/escalas/nova"><Plus /></Link> : null}</div><AuthMessage {...message} />
