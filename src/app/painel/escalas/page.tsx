...
export default async function SchedulesPage({ searchParams }: PageProps) {
  ...
  return <main className="mx-auto max-w-4xl px-4 py-7 sm:px-8"><div className="flex items-center justify-between gap-4"><h1 className="text-3xl font-bold">{view === "minhas" ? "Minhas escalas" : "Escalas do setor"}</h1>{canManageSchedules && view === "setor" ? <div className="flex items-center gap-2"><Link className="min-h-12 rounded-full border border-[var(--church-brand)] px-4 text-sm font-semibold leading-[3rem] text-[var(--church-brand)]" href="/painel/escalas/lote">Criar em lote</Link><Link aria-label="Nova escala" className="grid h-12 w-12 place-items-center rounded-full bg-[var(--church-brand)] text-white shadow-lg" href="/painel/escalas/nova"><Plus /></Link></div> : null}</div><AuthMessage {...message} />
