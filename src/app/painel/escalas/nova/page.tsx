...
export default async function NewSchedulePage({ searchParams }: PageProps) {
  ...
  const inputClass = "mt-2 min-h-12 w-full rounded-xl border border-[#d7dee7] bg-white px-4 text-gray-900 outline-none focus:border-[#277ad8] dark:bg-[#273136] dark:text-white dark:border-[#353e49]";
  return <main className="mx-auto max-w-3xl px-5 py-8 sm:px-8"><Link className="inline-flex items-center gap-2 font-semibold text-[#277ad8]" href="/painel/escalas"><ArrowLeft size={18} /> Escalas</Link><p className="mt-7 text-sm font-semibold uppercase tracking-[.15em] text-[#277ad8]">Nova escala</p><h1 className="mt-2 text-3xl font-bold">Dados do evento</h1><p className="mt-2 text-[#6b767d]">Ela ficará como rascunho até você montar a equipe e publicar.</p><AuthMessage {...message} />
    <form action={createSchedule} className="mt-8 grid gap-5 rounded-[1.75rem] bg-white p-6 shadow-sm sm:grid-cols-2">
      ...
      <button className="min-h-12 rounded-xl bg-[#277ad8] px-5 font-semibold text-white sm:col-span-2">Criar rascunho e montar equipe</button>
    </form>
  </main>;
}
