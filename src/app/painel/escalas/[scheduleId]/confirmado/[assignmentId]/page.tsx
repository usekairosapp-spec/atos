...
export default async function ConfirmedPage({ params }: PageProps) {
  ...
  return <main className="flex min-h-[calc(100vh-5rem)] items-center justify-center bg-[#11313d] p-4"><section className="flex min-h-[75vh] w-full max-w-xl flex-col items-center rounded-[2.5rem] bg-emerald-600 px-7 py-12 text-center text-white shadow-2xl"><span className="mt-8 grid h-36 w-36 place-items-center rounded-full bg-[#ffffff] text-emerald-600 shadow-xl"><Check size={76} strokeWidth={3} /></span><h1 className="mt-10 text-3xl font-bold">Presença confirmada!</h1><div className="mt-12 text-xl leading-relaxed"><p className="font-semibold">{confirmation.service_title}</p><p>{formatDate(date, tz)} · {formatTime(date, tz, { hour: "2-digit", minute: "2-digit" })}</p><p>{confirmation.department_name} — {confirmation.position_name}</p></div><p className="mt-10 text-lg">Deus abençoe seu ministério!</p>{calendarEvent?.html_link ? <a className="mt-8 flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-[#ffffff] px-5 font-bold text-emerald-700" href={calendarEvent.html_link} rel="noreferrer" target="_blank">Abrir no Google Agenda <ExternalLink size={18} /></a> : null}<form action={addAssignmentToGoogleCalendar} className="mt-4 w-full"><input type="hidden" name="assignmentId" value={assignmentId} /><input type="hidden" name="scheduleId" value={scheduleId} /><button className="flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl bg-[#ffffff] px-5 font-bold text-[#202e3c]"><CalendarPlus size={20} />{calendarEvent ? "Sincronizar ou recriar evento" : "Adicionar ao Google Agenda"}</button></form><Link className="mt-4 min-h-14 w-full rounded-2xl border border-white/70 px-5 py-4 font-bold text-white" href="/painel">Voltar para o início</Link></section></main>;
}
