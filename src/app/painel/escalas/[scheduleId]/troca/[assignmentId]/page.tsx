...
export default async function SwapPage({ params, searchParams }: PageProps) {
  ...
  return <main className="mx-auto max-w-2xl px-5 py-8 sm:px-8"><Link className="inline-flex items-center gap-2 text-lg font-bold" href={`/painel/escalas/${scheduleId}`}><ArrowLeft /> Solicitar troca</Link><AuthMessage {...message} /><section className="mt-7 rounded-[1.75rem] bg-gradient-to-br from-[var(--church-brand)] to-[var(--church-brand-dark)] p-6 text-white shadow-lg"><p className="text-sm font-semibold text-white/75">Sua escala</p><p className="mt-2 text-lg font-bold">{formatDate(startsAt, tz, { day: "2-digit", month: "2-digit" })} · {formatTime(startsAt, tz, { hour: "2-digit", minute: "2-digit" })}</p><p className="mt-2">{assignment.department_name} · {assignment.position_name}</p><p className="mt-1 text-sm text-white/75">{assignment.service_title}</p></section><SwapRequestForm assignmentId={assignment.assignment_id} candidates={candidates.sort((a, b) => a.name.localeCompare(b.name, "pt-BR"))} scheduleId={scheduleId} /></main>;
}
