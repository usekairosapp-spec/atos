  ...
  return (
    <main className="min-h-screen bg-[var(--background)] px-4 py-6 sm:px-8">
      <div className="mx-auto max-w-6xl">
        <header><p className="text-sm font-semibold uppercase tracking-[.15em] text-[#6827d8]">{roleTitle}</p><h1 className="mt-2 text-3xl font-bold">Olá, {firstName} 👋</h1></header>
        <AuthMessage {...message} />
        ...
        {platformRole && churchIds.length === 0 ? (
          <section className="mt-10 rounded-[1.75rem] bg-white p-7 shadow-sm">
            <p className="text-sm font-semibold uppercase tracking-[.15em] text-[#6827d8]">Primeira configuração</p>
            <h2 className="mt-3 text-2xl font-bold">Crie sua igreja</h2>
            <p className="mt-2 text-[#6f6b7d]">Os setores Técnica, Mídia, Oficiais, Louvor, Recepção e Kids serão criados automaticamente.</p>
            <form action={createChurch} className="mt-6 flex flex-col gap-3 sm:flex-row">
              ...
              <input className="min-h-14 flex-1 rounded-xl border border-[#dcd7e5] px-4" id="church-name" name="name" placeholder="Nome da igreja" required />
              <button className="min-h-14 rounded-xl bg-[#6827d8] px-6 font-semibold text-white" type="submit">Criar igreja</button>
            </form>
          </section>
        ) : null}
        {viewer.currentChurch ? (
          <section className="relative mt-8 overflow-hidden rounded-[2rem] bg-[linear-gradient(130deg,var(--church-brand),#20143e)] p-6 text-white shadow-xl" style={{ "--church-brand": viewer.churchBranding.primaryColor } as React.CSSProperties}>{viewer.churchBranding.coverUrl ? <div className="absolute inset-0 bg-cover bg-center opacity-30" style={{ backgroundImage: `url(${viewer.churchBranding.coverUrl})` }} /> : null}<div className="absolute inset-0 bg-gradient-to-r from-black/55 to-transparent" /><div className="relative flex items-end justify-between gap-4"><div><p className="text-sm text-white/70">Igreja atual</p><h2 className="text-2xl font-bold">{viewer.currentChurch.name}</h2></div>{viewer.role === "admin" ? <Link className="rounded-xl bg-white/95 px-4 py-3 font-semibold text-[#48208f] shadow-sm" href="/painel/membros">Gerenciar membros</Link> : null}</div></section>
        ) : null}
        ...
        {viewer.role === "leader" ? <section className="mt-8 grid gap-4 sm:grid-cols-2"><Link className="rounded-2xl bg-white p-5 shadow-sm" href="/painel/escalas"><ClipboardList className="text-[#6827d8]" /><strong className="mt-3 block">Gerenciar escalas</strong><span className="text-sm text-[#6f6b7d]">Somente dos seus setores</span></Link><Link className="rounded-2xl bg-white p-5 shadow-sm" href="/painel/comunicados"><Megaphone className="text-[#6827d8]" /><strong className="mt-3 block">Publicar comunicado</strong><span className="text-sm text-[#6f6b7d]">Avisos para sua equipe</span></Link></section> : null}
        {!platformRole && churchIds.length === 0 ? (
          <section className="mt-10 rounded-[1.75rem] bg-white p-7 shadow-sm">
            <h2 className="text-2xl font-bold">Entrar em uma igreja</h2>
            <p className="mt-2 text-[#6f6b7d]">{hasPendingMembership ? "Sua solicitação está aguardando aprovação." : "Solicite o código de convite à administradora."}</p>
            {!hasPendingMembership ? <form action={requestMembership} className="mt-6 flex flex-col gap-3 sm:flex-row"><label className="sr-only" htmlFor="invite-code">Código de convite</label><input className="min-h-14 flex-1 rounded-xl border border-[#dcd7e5] px-4 uppercase" id="invite-code" name="inviteCode" placeholder="CÓDIGO" required /><button className="min-h-14 rounded-xl bg-[#6827d8] px-6 font-semibold text-white">Solicitar entrada</button></form> : null}
          </section>
        ) : null}
        <section className="mt-10 grid gap-5 lg:grid-cols-2">
          {nextAssignment ? (() => { const schedule = Array.isArray(nextAssignment.department_schedules) ? nextAssignment.department_schedules[0] : nextAssignment.department_schedules; const service = Array.isArray(schedule.services) ? schedule.services[0] : schedule.services; const department = Array.isArray(schedule.departments) ? schedule.departments[0] : schedule.departments; const position = Array.isArray(nextAssignment.positions) ? nextAssignment.positions[0] : nextAssignment.positions; const date = new Date(service.starts_at); return <Link className="rounded-[1.75rem] bg-[linear-gradient(145deg,#6f31dc,#421797)] p-7 text-white shadow-xl" href={`/painel/escalas/${schedule.id}?visao=minhas`}><p className="text-violet-100">Sua próxima escala</p><div className="mt-5 flex items-center justify-between gap-5"><div><p className="text-sm font-semibold capitalize">{date.toLocaleDateString("pt-BR", { weekday: "long" })}</p><strong className="mt-1 block text-4xl">{date.toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" })}</strong><p className="mt-3 flex items-center gap-2"><Clock3 size={17} />{date.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</p></div><span className="grid h-24 w-24 place-items-center rounded-full border-2 border-white/70"><Camera size={42} /></span></div><h2 className="mt-5 text-center text-2xl font-bold">{department?.name}</h2><p className="text-center text-violet-100">{position?.name}</p><span className={`mx-auto mt-5 block w-fit rounded-full px-4 py-2 text-sm font-semibold ${nextAssignment.status === "confirmed" ? "bg-emerald-400/25 text-emerald-100" : "bg-white/15"}`}>{nextAssignment.status === "confirmed" ? "Confirmado" : "Aguardando confirmação"}</span></Link>; })() : <article className="rounded-[1.75rem] bg-[linear-gradient(145deg,#6f31dc,#421797)] p-7 text-white shadow-xl"><p className="text-violet-100">Sua próxima escala</p><h2 className="mt-4 text-2xl font-bold">Nenhuma escala publicada</h2><p className="mt-2 text-violet-100">Quando um líder publicar uma escala, ela aparecerá aqui.</p></article>}
          <Link className="rounded-[1.75rem] bg-white p-7 shadow-sm" href="/painel/calendario">
            <CalendarDays className="text-[#6827d8]" />
            <h2 className="mt-4 text-xl font-bold">{churchIds.length ? "Calendário" : "Aguardando vínculo"}</h2>
            <p className="mt-2 text-[#6f6b7d]">{churchIds.length ? "Consulte seus próximos compromissos e confirmações." : "A administradora precisa adicionar sua conta a uma igreja e aos setores."}</p>
          </Link>
        </section>
        <section className="mt-5 rounded-[1.75rem] border border-dashed border-[#cfc6df] bg-white/60 p-7 text-center">
          <UsersRound className="mx-auto text-[#8a8199]" />
