  ...
  return (
    <main className="min-h-screen bg-[var(--background)] px-4 py-6 sm:px-8">
      <div className="mx-auto max-w-6xl">
        <header><p className="text-sm font-semibold uppercase tracking-[.15em] text-[var(--church-brand)]">{roleTitle}</p><h1 className="mt-2 text-3xl font-bold">Olá, {firstName} 👋</h1></header>
        <AuthMessage {...message} />
        ...
        {platformRole && churchIds.length === 0 ? (
          <section className="mt-10 rounded-[1.75rem] bg-white p-7 shadow-sm">
            <p className="text-sm font-semibold uppercase tracking-[.15em] text-[var(--church-brand)]">Primeira configuração</p>
            ...
            <form action={createChurch} className="mt-6 flex flex-col gap-3 sm:flex-row">
              ...
              <button className="min-h-14 rounded-xl bg-[var(--church-brand)] px-6 font-semibold text-white" type="submit">Criar igreja</button>
            </form>
          </section>
        ) : null}
        {viewer.currentChurch ? (
          <section className="relative mt-8 overflow-hidden rounded-[2rem] bg-[linear-gradient(130deg,var(--church-brand),var(--church-brand-dark))] p-6 shadow-xl" style={{ "--church-brand": viewer.churchBranding.primaryColor } as React.CSSProperties}>{viewer.churchBranding.coverUrl ? <div className="absolute inset-0 bg-cover bg-center opacity-30" style={{ backgroundImage: `url(${viewer.churchBranding.coverUrl})` }} /> : null}<div className="absolute inset-0 bg-gradient-to-r from-black/55 to-transparent" /><div className="relative flex items-end justify-between gap-4"><div><p className="text-sm opacity-70" style={{ color: viewer.churchBranding.textColor }}>Igreja atual</p><h2 className="text-2xl font-bold" style={{ color: viewer.churchBranding.textColor }}>{viewer.currentChurch.name}</h2></div>{viewer.role === "admin" ? <Link className="rounded-xl bg-white/95 px-4 py-3 font-semibold text-[var(--church-brand)] shadow-sm" href="/painel/membros">Gerenciar membros</Link> : null}</div></section>
        ) : null}
        {viewer.role === "admin" && churchIds.length > 0 ? <section className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Link className="premium-card rounded-2xl p-5" href="/painel/membros"><UsersRound className="text-[var(--brand)]" /><strong className="mt-3 block">Membros e líderes</strong><span className="text-sm text-[var(--muted)]">Aprovações e permissões</span></Link><Link className="premium-card rounded-2xl p-5" href="/painel/setores"><ShieldCheck className="text-[var(--brand)]" /><strong className="mt-3 block">Setores</strong><span className="text-sm text-[var(--muted)]">Equipes da igreja</span></Link><Link className="premium-card rounded-2xl p-5" href={viewer.isLeader ? "/painel/escalas" : "/painel/escalas?visao=minhas"}><ClipboardList className="text-[var(--brand)]" /><strong className="mt-3 block">{viewer.isLeader ? "Gerenciar escalas" : "Minhas escalas"}</strong><span className="text-sm text-[var(--muted)]">{viewer.isLeader ? "Criar e editar escalas dos seus setores" : "Seus dias de serviço"}</span></Link><Link className="premium-card rounded-2xl p-5" href="/painel/comunicados"><Megaphone className="text-[var(--brand)]" /><strong className="mt-3 block">Publicar comunicado</strong><span className="text-sm text-[var(--muted)]">Avisos para sua equipe</span></Link>{viewer.isChurchAdmin ? <Link className="premium-card rounded-2xl p-5" href="/painel/igreja"><Palette className="text-[var(--brand)]" /><strong className="mt-3 block">Identidade visual</strong><span className="text-sm text-[var(--muted)]">Logo, capa e cor</span></Link> : null}</section> : null}
        {viewer.role === "leader" ? <section className="mt-8 grid gap-4 sm:grid-cols-2"><Link className="rounded-2xl bg-white p-5 shadow-sm" href="/painel/escalas"><ClipboardList className="text-[var(--church-brand)]" /><strong className="mt-3 block">Gerenciar escalas</strong><span className="text-sm text-[#6b767d]">Somente dos seus setores</span></Link><Link className="rounded-2xl bg-white p-5 shadow-sm" href="/painel/comunicados"><Megaphone className="text-[var(--church-brand)]" /><strong className="mt-3 block">Publicar comunicado</strong><span className="text-sm text-[#6b767d]">Avisos para sua equipe</span></Link></section> : null}
        {!platformRole && churchIds.length === 0 ? (
          <section className="mt-10 rounded-[1.75rem] bg-white p-7 shadow-sm">
            ...
            {!hasPendingMembership ? <form action={requestMembership} className="mt-6 flex flex-col gap-3 sm:flex-row"><label className="sr-only" htmlFor="invite-code">Código de convite</label><input className="min-h-14 flex-1 rounded-xl border border-[#d7dee5] px-4 uppercase" id="invite-code" name="inviteCode" placeholder="CÓDIGO" required /><button className="min-h-14 rounded-xl bg-[var(--church-brand)] px-6 font-semibold text-white">Solicitar entrada</button></form> : null}
          </section>
        ) : null}
        <section className="mt-10 grid gap-5 lg:grid-cols-2">
          {nextAssignment ? (() => { const schedule = Array.isArray(nextAssignment.department_schedules) ? nextAssignment.department_schedules[0] : nextAssignment.department_schedules; const service = Array.isArray(schedule.services) ? schedule.services[0] : schedule.services; const department = Array.isArray(schedule.departments) ? schedule.departments[0] : schedule.departments; const position = Array.isArray(nextAssignment.positions) ? nextAssignment.positions[0] : nextAssignment.positions; const date = new Date(service.starts_at); const tz = viewer.profile.timezone; return <Link className="rounded-[1.75rem] bg-[linear-gradient(145deg,var(--church-brand),var(--church-brand-dark))] p-7 text-white shadow-xl" href={`/painel/escalas/${schedule.id}?visao=minhas`}><p className="text-white/75">Sua próxima escala</p><div className="mt-5 flex items-center justify-between gap-5"><div><p className="text-sm font-semibold capitalize">{formatDate(date, tz, { weekday: "long" })}</p><strong className="mt-1 block text-4xl">{formatDate(date, tz, { day: "2-digit", month: "2-digit" })}</strong><p className="mt-3 flex items-center gap-2"><Clock3 size={17} />{formatTime(date, tz, { hour: "2-digit", minute: "2-digit" })}</p></div><span className="grid h-24 w-24 place-items-center rounded-full border-2 border-white/70"><Camera size={42} /></span></div><h2 className="mt-5 text-center text-2xl font-bold">{department?.name}</h2><p className="text-center text-white/75">{position?.name}</p><span className={`mx-auto mt-5 block w-fit rounded-full px-4 py-2 text-sm font-semibold ${nextAssignment.status === "confirmed" ? "bg-emerald-400/25 text-emerald-100" : "bg-white/15"}`}>{nextAssignment.status === "confirmed" ? "Confirmado" : "Aguardando confirmação"}</span></Link>; })() : <article className="rounded-[1.75rem] bg-[linear-gradient(145deg,var(--church-brand),var(--church-brand-dark))] p-7 text-white shadow-xl"><p className="text-white/75">Sua próxima escala</p><h2 className="mt-4 text-2xl font-bold">Nenhuma escala publicada</h2><p className="mt-2 text-white/75">Quando um líder publicar uma escala, ela aparecerá aqui.</p></article>}
          <Link className="rounded-[1.75rem] bg-white p-7 shadow-sm" href="/painel/calendario">
            <CalendarDays className="text-[var(--church-brand)]" />
