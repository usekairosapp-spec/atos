...
export default async function ChurchBrandingPage({ searchParams }: PageProps) {
  ...
  return <main className="mx-auto max-w-4xl px-5 py-8 sm:px-8">
    ...
    <section className="premium-card mt-8 overflow-hidden rounded-[2rem]">
      <div className="relative h-48 bg-[linear-gradient(135deg,var(--church-color),#11222d)]" style={{ "--church-color": viewer.churchBranding.primaryColor } as React.CSSProperties}>{viewer.churchBranding.coverUrl ? <Image alt="Capa atual da igreja" className="object-cover opacity-75" fill sizes="900px" src={viewer.churchBranding.coverUrl} unoptimized /> : null}<div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" /><div className="absolute bottom-5 left-5 flex items-center gap-4">{viewer.churchBranding.logoUrl ? <Image alt="Logo atual" className="h-16 w-16 rounded-2xl bg-white object-contain p-1" height={64} src={viewer.churchBranding.logoUrl} width={64} unoptimized /> : <span className="rounded-2xl bg-white p-3"><KairosBrand compact /></span>}<strong className="text-2xl text-white">{viewer.currentChurch.name}</strong></div></div>
