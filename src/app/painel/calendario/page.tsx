...
export default async function CalendarPage({ searchParams }: PageProps) {
  ...
  return <main className="mx-auto max-w-4xl px-4 py-7 sm:px-8">
    <div className="flex items-end justify-between gap-4"><div><p className="text-sm font-semibold uppercase tracking-[.15em] text-[#277ad8]">Minha agenda</p><h1 className="mt-1 text-3xl font-bold">Calendário</h1></div><span className="rounded-full bg-blue-100 px-3 py-2 text-sm font-semibold text-[#277ad8]">{monthAssignments.length} {monthAssignments.length === 1 ? "escala" : "escalas"}</span></div>
    ...
    <section className="mt-7 rounded-[2rem] bg-white p-4 shadow-sm sm:p-7">
      <div className="flex items-center justify-between"><Link aria-label="Mês anterior" className="grid h-11 w-11 place-items-center rounded-full hover:bg-blue-50" href={`/painel/calendario?mes=${previous}`}><ChevronLeft /></Link><h2 className="text-lg font-bold capitalize">{first.toLocaleDateString("pt-BR", { month: "long", year: "numeric", timeZone: TIME_ZONE })}</h2><Link aria-label="Próximo mês" className="grid h-11 w-11 place-items-center rounded-full hover:bg-blue-50" href={`/painel/calendario?mes=${next}`}><ChevronRight /></Link></div>
      ...
      <div className="grid grid-cols-7 gap-y-2 text-center">{cells.map((day, index) => {
        ...
        return <Link aria-current={selected ? "date" : undefined} aria-label={`${accessibleDate}, ${countLabel}${dateKey === todayKey ? ", hoje" : ""}`} className="grid min-h-12 place-items-center" href={`/painel/calendario?mes=${currentMonth}&dia=${dateKey}`} key={dateKey}><span className={`relative grid h-11 w-11 place-items-center rounded-full font-semibold transition ${selected ? "bg-[#16394d] text-white shadow-md" : count ? "bg-blue-100 text-[#277ad8]" : dateKey === todayKey ? "bg-emerald-100 text-emerald-800" : "hover:bg-[#f1f4f8]"} ${dateKey === todayKey && !selected ? "ring-2 ring-emerald-500 ring-offset-2" : ""}`}>{day}{count > 1 ? <small aria-hidden="true" className={`absolute -right-1 -top-1 grid h-5 min-w-5 place-items-center rounded-full px-1 text-[10px] ${selected ? "bg-white text-[#16394d]" : "bg-[#277ad8] text-white"}`}>{count}</small> : null}</span></Link>;
      })}</div>
      <div className="mt-5 flex items-center justify-center gap-2 text-sm text-[#6b767d]"><i className="h-3 w-3 rounded-full bg-blue-200" />Tem escala</div>
    </section>
