    ...
    <main className="min-h-screen bg-[var(--background)] px-4 py-6 sm:px-8">
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mt-6 rounded-[1.5rem] bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-[#6b767d]">Cada setor pode ter dois ou mais líderes. Ao designar alguém, o painel dessa pessoa muda automaticamente.</p>
          <form action={assignDepartment} className="mt-5 grid gap-4">
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="font-semibold">Pessoa<select className="mt-2 min-h-12 w-full rounded-xl border border-[#d7dee7] px-3" name="userId" required><option value="">Selecione o membro</option>{activeMembers.map((item) => { const profile = item.profiles as ProfileRelation; const name = Array.isArray(profile) ? profile[0]?.full_name : profile?.full_name; return <option value={item.user_id} key={item.user_id}>{name || "Membro"}</option>; })}</select></label>
        ...
        <section className="mt-6 rounded-[1.5rem] bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-[#6b767d]">Retirar a liderança mantém a pessoa como membro do setor e atualiza o painel dela.</p>
          <div className="mt-4 divide-y divide-[#e8ecf1]">
            {activeLeaders.map((leader) => <article className="flex flex-col justify-between gap-4 py-4 sm:flex-row sm:items-center" key={`${leader.department_id}-${leader.user_id}`}><div><p className="font-semibold">{memberNames.get(leader.user_id) || "Líder"}</p><p className="text-sm text-[#6b767d]">{departmentNames.get(leader.department_id)}</p></div><form action={removeDepartmentLeadership}><input name="departmentId" type="hidden" value={leader.department_id} /><input name="userId" type="hidden" value={leader.user_id} /><ConfirmSubmitButton confirmation={`Retirar ${memberNames.get(leader.user_id) || "esta pessoa"} da liderança de ${departmentNames.get(leader.department_id)}?`} className="min-h-11 rounded-xl border border-amber-300 px-4 font-semibold text-amber-800" pendingLabel="Atualizando...">Retirar liderança</ConfirmSubmitButton></form></article>)}
            {!activeLeaders.length ? <p className="py-6 text-[#6b767d]">Nenhum líder de setor definido.</p> : null}
          </div>
        </section>

        <section className="mt-6 rounded-[1.5rem] bg-white p-6 shadow-sm">
          <h2 className="text-xl font-bold">Membros ativos</h2>
          <p className="mt-1 text-sm text-[#6b767d]">A remoção encerra o acesso aos setores e cancela participações futuras, preservando o histórico.</p>
          <div className="mt-4 divide-y divide-[#e8ecf1]">
