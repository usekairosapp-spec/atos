...
export default async function DepartmentsPage({ searchParams }: PageProps) {
  ...
  return <main className="mx-auto max-w-6xl px-5 py-8 sm:px-8">
    <p className="text-sm font-semibold uppercase tracking-[.15em] text-[#6827d8]">Equipes</p>
    ...
    {viewer.role === "admin" ? <section className="mt-8 rounded-2xl bg-white p-6 shadow-sm">
      ...
      <p className="mt-1 text-sm text-[#6f6b7d]">Adicione um novo setor à sua igreja. Você poderá gerenciar funções e membros após criá-lo.</p>
      <form action={createDepartment} className="mt-4 flex flex-col gap-3 sm:flex-row">
        <input className="min-h-12 flex-1 rounded-xl border border-[#ddd7e7] px-4 outline-none focus:border-[#6827d8]" name="name" placeholder="Ex.: Som, Louvor, Técnica" required />
        <button className="min-h-12 rounded-xl bg-[#6827d8] px-5 font-semibold text-white"><Plus className="mr-2 inline" size={18} />Criar setor</button>
      </form>
    </section> : null}
    ...
    <section className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      {departments?.map((department) => <Link className="rounded-2xl bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-md" href={`/painel/setores/${department.id}`} key={department.id}>
        ...
        <p className="mt-2 text-sm text-[#6f6b7d]">{viewer.role === "admin" ? "Consultar funções do setor" : "Gerenciar funções do seu setor"}</p>
      </Link>)}
    </section>
  </main>;
}
