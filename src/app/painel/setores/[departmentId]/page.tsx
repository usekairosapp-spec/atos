...
export default async function DepartmentDetailPage({ params, searchParams }: PageProps) {
  ...
  return (
    <main className="mx-auto max-w-5xl px-5 py-8 sm:px-8">
      <Link className="inline-flex items-center gap-2 font-semibold text-[#6827d8]" href="/painel/setores"><ArrowLeft size={18} /> Setores</Link>
      <p className="mt-7 text-sm font-semibold uppercase tracking-[.15em] text-[#6827d8]">Funções do setor</p><h1 className="mt-2 text-3xl font-bold">{department.name}</h1>
      <AuthMessage {...message} />
